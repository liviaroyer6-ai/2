...
def collision_overlay(scr, bg, level, progress):
    ...
    while True:
        ...
        pygame.draw.rect(scr, BLUE, menu_button)
...
def game_loop(level, progress):
    ...
    print("[DEBUG] Checking for selected character")
    ...
    try:
        ...
        enemies = [enemy_cls(load_image(os.path.join(IMAGE_DIR, CHAR_SPRITES[1])), speed) for _ in range(count)]

        unb = UnbreakableEnemy(400, 100, 40, 40, YELLOW)
        pu = PowerUp()
        boss = None
        laser_path = os.path.join(SOUND_DIR, "laser.mp3")
        shoot_sound = pygame.mixer.Sound(laser_path)
        ...
        while True:
            ...
            if not boss_active:
                ...
                if not boss_active and score >= thresh:
                    boss_active = True
                    boss_entry_delay = pygame.time.get_ticks() + 1000
...
def selection_screen(progress=None):
    ...
    chars = [load_image(os.path.join(IMAGE_DIR, fn)) for fn in CHAR_SPRITES]
    idx_c = 0
    pulse_timer = 0
    done = False

    try:
        bg_path = os.path.join(IMAGE_DIR, "ui_background_characters.jpg")
    ...
    def confirm_selection():
        ...
        selected = load_image(os.path.join(IMAGE_DIR, CHAR_SPRITES[idx_c]))
        if selected:
